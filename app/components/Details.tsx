  ...
  return (
    <>
      ...
      <div>
        <button
          onClick={handleback}
          className="p-4 flex items-center space-x-2 text-[rgba(37,50,75,1)] font-poppins font-semibold text-lg mb-4 gap-2"
        >
          &lt; <span>Back</span>
        </button>
          ...
          <div className="space-y-5 w-1/4">
            ...
            <div className="flex">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {Date_format(curr_job?.datePosted ?? "")}
                </p>
              </div>
            </div>
            <div className="flex">
            ...
            <div className="flex">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {Date_format(curr_job?.startDate ?? "")}{" "}
                </p>
              </div>
            </div>
            <div className="flex">
              ...
              <div>
                ...
                <p className="font-semibold">
                  {Date_format(curr_job?.endDate ?? "")}
                </p>
              </div>
            </div>
            <hr />
            <p className="font-poppins font-black text-2xl text-[rgba(37,50,75,1)]">
              {" "}
              Categories
            </p>
            ...
            {curr_job?.categories?.map((cat: string, index: number) => {
              if (index % 2 == 0) {
                return (
                  <p className="inline-block bg-[rgba(235,133,51,0.1)] text-[rgba(255,184,54,1)] font-Epilogue font-semibold py-1 px-3 w-fit  rounded-full m-2 bg-center">
                    {" "}
                    {cat}
                  </p>
                );
            ...
            {curr_job?.requiredSkills?.map((skill: string, index: number) => {
              return (
                <p className="bg-[rgba(248,248,253,1)] text-[#4540def5] font-normal py-1 px-3 w-fit m-2 inline-block">
                  {skill}
                </p>
              );
            })}
